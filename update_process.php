...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ids'])) {
    $ids = $_POST['ids']; // Array of selected row IDs

    foreach ($ids as $index => $id) {
        $updateFields = [];

        foreach ($_POST as $key => $values) {
            if ($key !== 'ids' && isset($values[$index])) {
                $safeValue = $conn->real_escape_string($values[$index]);
    ...
    $conn->close();
    header("Location: admin.php"); // Redirect to main page after update
    exit();
}
?>
